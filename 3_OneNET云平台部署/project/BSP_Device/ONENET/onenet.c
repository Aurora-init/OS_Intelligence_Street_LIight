...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	if(flag_change == 1)								//温度Temp数据流
	{
		printf("\r\n本次输出温度: %d.%d\r\n",0,0);
		strcpy(buf, "{");
		memset(text, 0, sizeof(text));
		sprintf(text, "\"Temp\":%d.%d",0,0);
		strcat(buf, text);
		strcat(buf, "}");
		flag_change = 2;
	}
	if(flag_change == 0)								//湿度Humi数据流
	{
		printf("\r\n本次输出湿度: %d.%d\r\n",0,0);
		strcpy(buf, "{");
		memset(text, 0, sizeof(text));
		sprintf(text, "\"Humi\":%d.%d",0,0);
		strcat(buf, text);
		strcat(buf, "}");
		flag_change = 1;
	}
...
void OneNet_SendData(void)
{
	...
	UsartPrintf(&huart1, "Tips:	OneNet_SendData-MQTT\r\n");
	...
	if(body_len)
	{
		if(MQTT_PacketSaveData(DEVID, body_len, NULL, 3, &mqttPacket) == 0)					//封包				
		{
			...
			UsartPrintf(&huart1, "WARN:	EDP_NewBuffer Failed\r\n");
	}
}
